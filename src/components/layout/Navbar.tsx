  ...
  return (
    <header className="w-full bg-white border-b border-gray-100 sticky top-0 z-50">
      ...
      <div className="w-full border-b border-gray-50 bg-white">
        <div className="px-4 py-4 md:py-6 flex items-center justify-between gap-4 max-w-[1920px] mx-auto">
          {/* Logo & Company Info */}
          <Link href="/" className="flex items-center group shrink-0">
            <div className="relative w-12 h-12 md:w-20 md:h-20 flex items-center justify-center bg-white rounded-xl shadow-sm group-hover:shadow-md transition-all p-1 border border-gray-50">
              <Image
            ...
            <div className="ml-3 hidden sm:block">
              <div className="text-xl md:text-3xl font-black text-gray-900 tracking-tighter leading-none uppercase">Hat Traders</div>
              <div className="text-[9px] md:text-[11px] text-gray-400 font-bold uppercase tracking-widest mt-1">Complete Construction Solutions</div>
            </div>
          </Link>
          ...
          <div className="hidden md:flex flex-1 max-w-2xl mx-8">
            <form onSubmit={handleSearch} className="relative w-full group">
              <input
                type="text"
                placeholder="Search everything for your project (e.g. Nippon, Grinder, Bluebird)..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-14 pr-4 py-3.5 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-orange-500/10 focus:bg-white focus:border-orange-500 transition-all text-sm outline-none shadow-sm"
              />
              ...
              <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 bg-orange-500 text-white px-6 py-2 rounded-xl text-xs font-black hover:bg-orange-600 transition-all active:scale-95">
                SEARCH
              </button>
            </form>
          </div>
          ...
          <div className="hidden md:flex items-center gap-3 shrink-0">
            ...
            <div className="flex flex-col">
              ...
              <a href="tel:+923001234567" className="text-sm md:text-lg font-black text-gray-900 hover:text-orange-500 transition-colors tracking-tight">
                +92 300 123 4567
              </a>
            </div>
          </div>
      ...
      <div className="w-full bg-white shadow-sm md:block hidden">
        <nav className="flex items-center justify-center gap-2 py-0 max-w-[1920px] mx-auto">
          {navItems.map((item, idx) => (
            ...
            >
              <Link
                href={item.href}
                className={cn(
                  "px-4 xl:px-8 py-5 text-xs xl:text-[13px] font-black text-gray-700 hover:text-orange-600 transition-all relative flex items-center gap-1.5 uppercase tracking-wide",
              ...
              >
                ...
                {item.dropdown && (
                  <ChevronDown className={cn(
                    "w-4 h-4 transition-transform duration-300 opacity-40 group-hover:opacity-100",
                    activeDropdown === item.name && "rotate-180"
                  )} />
                )}
                {/* Active Indicator */}
                <div className={cn(
                  "absolute bottom-0 left-0 w-full h-1 bg-orange-500 scale-x-0 transition-transform duration-300 origin-center",
