...
export async function generateMetadata({
  ...
  const product = duluxProducts.find((p) => p.slug === productSlug);
  
  if (!product) return { title: "Product Not Found" };
  
  return {
    title: `${product.title} - ICI Dulux Paint | Hat Traders`,
    description: product.shortDesc,
    alternates: {
      canonical: `https://hattraders.com/products/decorative-paints/ici-dulux-paints/${productSlug}`,
    },
  };
}

export function generateStaticParams() {
  return duluxProducts.map((p) => ({ productSlug: p.slug }));
}

export default async function DuluxProductDetailPage({
  params,
}: {
  params: Promise<Params>;
}) {
      ...
      <div className="bg-gray-50 border-b border-gray-200">
        <div className="container mx-auto px-4 py-4">
          <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm font-medium text-gray-500">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <Link href="/products/decorative-paints/ici-dulux-paints" className="hover:text-gray-900 transition-colors">ICI Dulux Paints</Link>
            <ChevronRight className="w-3.5 h-3.5" />
