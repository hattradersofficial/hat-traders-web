...
export async function generateMetadata({
  ...
  const product = sparcoProducts.find((p) => p.slug === productSlug);
  
  if (!product) return { title: "Product Not Found" };
  
  return {
    title: `${product.title} - Sparco Paint | Hat Traders`,
    description: product.shortDesc,
    alternates: {
      canonical: `https://hattraders.com/products/decorative-paints/sparco-paints/${productSlug}`,
    },
  };
}

export function generateStaticParams() {
  return sparcoProducts.map((p) => ({ productSlug: p.slug }));
}

export default async function SparcoProductDetailPage({
  params,
}: {
  params: Promise<Params>;
}) {
      ...
      <div className="bg-gray-50 border-b border-gray-200">
        <div className="container mx-auto px-4 py-4">
          <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm font-medium text-gray-500">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <Link href="/products/decorative-paints/sparco-paints" className="hover:text-gray-900 transition-colors">Sparco Paints</Link>
            <ChevronRight className="w-3.5 h-3.5" />
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
            ...
            <div className="w-full lg:w-1/2 flex flex-col justify-center">
              <div className="inline-flex items-center gap-2 bg-red-100 text-red-700 text-xs font-bold px-3 py-1 rounded-full mb-6 uppercase tracking-wider w-max">
                Sparco Paint
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-auto">
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white font-bold px-8 py-4 rounded-xl transition-all shadow-lg shadow-red-500/30 hover:-translate-y-0.5"
                >
                  <Phone className="w-5 h-5" /> Request Quote
                </a>
                <a
                  href={`https://wa.me/+923395566700?text=${encodeURIComponent(`Hi, I'm interested in ${product.title} and want to know more about it.`)}`}
                  target="_blank"
