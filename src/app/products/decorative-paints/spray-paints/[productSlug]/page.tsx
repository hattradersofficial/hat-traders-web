...
export async function generateMetadata({
  ...
  const product = sprayPaints.find((p) => p.slug === productSlug);
  
  if (!product) return { title: "Product Not Found" };
  
  return {
    title: `${product.title} - Spray Paint | Hat Traders`,
    description: product.shortDesc,
    alternates: {
      canonical: `https://hattraders.com/products/decorative-paints/spray-paints/${productSlug}`,
    },
  };
}

export function generateStaticParams() {
  return sprayPaints.map((p) => ({ productSlug: p.slug }));
}

export default async function SprayProductDetailPage({
  params,
}: {
  params: Promise<Params>;
}) {
      ...
      <div className="bg-gray-50 border-b border-gray-200">
        <div className="container mx-auto px-4 py-4">
          <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm font-medium text-gray-500">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <Link href="/products/decorative-paints/spray-paints" className="hover:text-gray-900 transition-colors">Spray Paints</Link>
            <ChevronRight className="w-3.5 h-3.5" />
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
            ...
            <div className="w-full lg:w-1/2 flex flex-col justify-center">
              <div className="inline-flex items-center gap-2 bg-slate-100 text-slate-700 text-xs font-bold px-3 py-1 rounded-full mb-6 uppercase tracking-wider w-max">
                Spray Paint
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-auto">
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-900 text-white font-bold px-8 py-4 rounded-xl transition-all shadow-lg shadow-slate-500/30 hover:-translate-y-0.5"
                >
