...
async function scrapePorta() {
  ...
  await page.goto('https://porta.pk/product-category/bathroom/sanitary-ware/', { waitUntil: 'domcontentloaded' });
  ...
  if (productLinks.length < 20) {
    ...
    await page.goto('https://porta.pk/product-category/bathroom/faucets/', { waitUntil: 'domcontentloaded' });
    const newLinks = await page.evaluate(() => {
      const anchors = Array.from(document.querySelectorAll('a[href*="/product/"]:not([href*="category"])'));
      return anchors.map(a => a.href).filter(href => href && href.includes('/product/'));
    });
  ...
  fs.writeFileSync(`${destDir}/portaProducts.ts`, fileContent);
  console.log(`Saved to ${destDir}/portaProducts.ts`);
  
  await browser.close();
}

scrapePorta().catch(async (e) => {
  console.error(e);
  process.exit(1);
});
